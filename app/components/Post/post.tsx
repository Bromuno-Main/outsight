...
// --- PostSmall ---
export const PostSmall: React.FC<Post> = ({ title, author, date, duration, image, index, body, isHot, tags }) => {
    // Removed unused excerpt logic

    return (
        // Added group for potential hover effects on title/author
        <article className="flex items-start gap-4 p-4 bg-[#F8F1E6] rounded-3xl transition-colors group cursor-pointer">
            ...
            <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap whitespace-nowrap gap-1 mb-2"> {/* Use flex-wrap and smaller gap */}
                    {tags.slice(0, 3).map((tag) => ( // Limit tags shown if needed
                        <span key={tag} className="text-[.7rem] px-2 py-0.5 bg-black/5 rounded-full">
                            {tag}
                        </span>
                    ))}
                </div>
                ...
                <div className="flex items-center gap-x-2 gap-y-1 text-xs sm:text-sm text-gray-600 mt-2 flex-wrap"> {/* Adjusted gaps and text size */}
                    ...
                    <Link
                        ...
                        className="hover:underline hover:text-black transition-colors font-medium" // Added font-medium
...
export const PostLarge: React.FC<Post> = ({ title, author, duration, date, body, image, tags }) => {
  return (
    <article className="p-8 rounded-lg flex flex-col h-full">
      ...
      <div className=" flex flex-col flex-1 ">
        <div className="flex flex-wrap gap-2 mb-4">
          {tags.map((tag) => (
            <span key={tag} className="text-sm px-3 py-1 bg-black/5 rounded-full">{tag}</span>
          ))}
        </div>
